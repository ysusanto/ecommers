        ...
        <div class="customer-login-register">
          ...
          <div class="login-form">
            <form action="<?php echo site_url('site/login'); ?>" id="login_form" method="post">
              <input type="hidden" name="preview_url" value="<?php if (isset($_SERVER['HTTP_REFERER']) && $this->session->userdata('single_pre_url') == '') {
                                                                echo str_replace(base_url() . 'site/register', '', $_SERVER['HTTP_REFERER']);
                                                              } else {
                                                                echo $this->session->userdata('single_pre_url');
                                                                $this->session->unset_userdata('single_pre_url');
                                                              } ?>">
        ...
        <div class="customer-login-register register-pt-0">
          ...
          <div class="register-form">
            ...
            <form action="<?php echo site_url('site/register'); ?>" id="registerForm" method="post">
              <input type="hidden" name="preview_url" value="<?php if (isset($_SERVER['HTTP_REFERER'])) {
      ...
      <div class="modal-body" style="padding-top: 10px">
        <div class="modal-details">
          <form action="<?= site_url('site/reset_password') ?>" method="post" id="lost_password_form">
            ...
            <div class="login-submit">
              <button type="submit" class="form-button"><?= $this->lang->line('reset_password_send_btn') ?></button>
            </div>
          </form>
        </div>
      </div>
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          if (obj.status == '1') {
            ...
            swal({
              title: "<?= $this->lang->line('error_lbl') ?>",
              text: obj.message,
              type: 'error'
            });
          }
  ...
  $(".btn_resend").on("click", function(e) {
    ...
    $.ajax({
      ...
      success: function(res) {
        ...
        $(".process_loader").hide();
        swal({
          title: "<?= $this->lang->line('resent_lbl') ?>",
          text: obj.msg,
          type: "success"
        });
        $("#countdown").html("60");
