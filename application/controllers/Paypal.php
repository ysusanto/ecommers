    ...
    function getPaymentStatus()
    {
        ...
        if ($result->getState() == 'approved') {
            ...
            if ($this->input->post('buy_now') == 'false'){
                ...
                $row_items = $this->common_model->selectByids(array('order_id' => $order_id), 'tbl_order_items');

                foreach ($row_items as $key => $value) {
                    
                    $this->common_model->deleteByids(array('user_id' => $this->session->userdata('user_id'), 'product_id' => $value->product_id), 'tbl_cart_tmp');

                    $this->common_model->deleteByids(array('user_id' => $this->session->userdata('user_id'), 'product_id'  =>  $value->product_id),'tbl_cart');
                }
            }
